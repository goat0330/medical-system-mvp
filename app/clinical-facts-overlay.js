...
function renderGroupingFacts(context){
  ...
  const secondary=context.collections?.['diagnosis.secondary']||[];const procedures=context.collections?.['procedure.others']||[];
  for(const item of [...secondary,...procedures])for(const id of item.evidenceRefs||[])evidenceIds.add(id);
  ...
  const collectionConflicts=context.collectionConflicts||[];
  return `<details class="fact-platform-card op-data-details" id="grouping-source-details" data-fact-platform-card><summary>患者事实与证据 · ${facts.length+secondary.length+procedures.length} 个分组事实${conflicts.length+collectionConflicts.length?` · ${conflicts.length+collectionConflicts.length} 项待确认`:''}</summary><div class="op-data-details__body"><div class="fact-key-grid">${groupingFactCell(context,'patient.sex','性别')}${groupingFactCell(context,'patient.age','年龄')}${groupingFactCell(context,'diagnosis.principal.code','主要诊断编码','diagnosis.principal.name')}${groupingFactCell(context,'procedure.primary.code','主要手术/操作编码','procedure.primary.name')}</div><div class="op-collection-facts"><b>其他诊断</b><span>${secondary.map((item)=>`${esc(item.code||'未编码')} ${esc(item.name)}`).join('；')||'无'}</span><b>其他手术 / 操作</b><span>${procedures.map((item)=>`${esc(item.code||'未编码')} ${esc(item.name)}`).join('；')||'无'}</span></div>${conflicts.length?`<div class="fact-conflict-list"><h4>待确认的关键事实</h4>${conflicts.map(conflictHtml).join('')}</div>`:''}${collectionConflicts.length?`<div class="fact-conflict-list"><h4>列表型事实编码冲突</h4>${collectionConflicts.map((conflict)=>`<div class="fact-conflict is-blocking"><b>${esc(conflict.collection==='diagnosis.secondary'?'其他诊断':'其他手术/操作')}</b><p>${esc(conflict.reason)}</p><p>候选编码：${conflict.candidates.map((candidate)=>esc(candidate.value)).join('、')}</p><small>候选已保留在左侧输入列表；请核实并删除不正确项后重新分组。</small></div>`).join('')}</div>`:''}${!conflicts.length&&!collectionConflicts.length?'<div class="fact-ok">未发现待确认的分组关键事实冲突。</div>':''}<details class="fact-evidence-list"><summary>查看字段证据（${evidence.length}）</summary>${evidence.map((item)=>`<div><b>${esc(item.fieldName||item.metadata?.templateName||item.sourceType)}</b><span>${esc(item.sourceDocumentType||item.sourceType)}${item.sourceVersion?` · ${esc(item.sourceVersion)}`:''}</span><p>${esc(item.excerpt)}</p></div>`).join('')}</details><small>事实修订 ${esc(context.revision)} · 证据索引 ${context.evidence.length}</small></div></details>`;
}
