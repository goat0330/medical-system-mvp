...
function apply(){
  ...
  applying=true;try{const context=buildClinicalFactContext({episode,documentSnapshots:documentsFor(episode)});const anchor=content.querySelector('.op-source')||content.firstElementChild;if(anchor)anchor.insertAdjacentHTML('afterend',render(context));else content.insertAdjacentHTML('afterbegin',render(context));const root=content.querySelector('[data-fact-platform-card]');if(root)bind(root,episode.episodeId);}finally{applying=false;}
}
