...
export function runRealBusinessPipeline({episode,settlement,settlementIssues=[],policyProfileId=null,overrideMethod=null,confirmedMappings=[],localPaymentParameters=null}={}){
  ...
  else if(!route.grouperReady)groupingResult={status:'GROUPER_NOT_AVAILABLE',method:route.paymentMethod,version:route.grouperVersion||null,group:null,trace:[{stage:'GROUPER_PROFILE_VALIDATION',status:'BLOCKED',input:route.displayName||route.id,output:'未执行分组',rule:route.reason||'当前方案对应的分组规则尚未接入。'}]};
  ...
  if(groupingResult.group&&(groupingResult.grouperSystem!==route.grouperSystem||groupingResult.version!==route.grouperVersion))groupingResult={status:'GROUPER_PROFILE_VERSION_MISMATCH',method:route.paymentMethod,version:groupingResult.version||null,group:null,trace:[{stage:'GROUPER_PROFILE_VALIDATION',status:'BLOCKED',input:`${groupingResult.grouperSystem||'未知'} / ${groupingResult.version||'未知版本'}`,output:`期望 ${route.grouperSystem} / ${route.grouperVersion}`,rule:'执行器与支付/分组方案不完全匹配，拒绝使用该结果。'}]};
