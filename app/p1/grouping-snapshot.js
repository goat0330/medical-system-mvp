...
export function buildGroupingSnapshot({ episode, settlement }) {
  ...
  const secondary = episode?.diagnoses?.secondary ?? settlement?.inpatient?.secondaryDiagnoses ?? [];
  const primaryProcedure = projection.principalProcedure || null;
  const otherProcedures = episode?.procedures?.slice?.(1) ?? settlement?.procedures?.others ?? [];
  ...
  const collectionConflicts=(context.collectionConflicts||[]).filter((conflict)=>inputSources[conflict.collection]?.status!=='MANUAL_OVERRIDE');
  const blockingConflicts=[...context.conflicts.filter((x)=>x.blocking&&x.impactScope.includes('GROUPING')),...collectionConflicts];
  ...
  return {
    ...
    secondaryDiagnoses: secondary.map((x)=>({code:x.code,name:x.name,factRefs:x.factRefs,sourceStatus:x.sourceStatus})),
    ...
    otherProcedures:otherProcedures.map((x)=>({code:x.code,name:x.name,factRefs:x.factRefs,sourceStatus:x.sourceStatus})),
    ...
    quality:{conflicts:[...context.conflicts.filter((x)=>x.impactScope.includes('GROUPING')),...collectionConflicts],blockingConflicts},
