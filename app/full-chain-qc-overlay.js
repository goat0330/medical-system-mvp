...
function render(run, groupingOnly=false){
  if(groupingOnly){
    ...
    return `<section class="base-card full-qc-card" data-full-chain-qc><div class="base-card__header"><div><h3>分组前质控</h3><p>${esc(summary)}仅展示影响当前 DRG/DIP 的事项。</p></div><div class="full-qc-head-status"><strong>${relevant.length}</strong><span>影响分组</span><em class="status-badge status-badge--${tone(status)} status-badge--compact">${esc(groupingStatusLabel(status))}</em></div></div>${relevant.length?`<div class="full-qc-issues">${relevant.map((issue)=>issueRow(issue,true)).join('')}</div>`:'<div class="fact-ok">关键输入检查无已知阻断项；请执行院内预分组查看规则结果。</div>'}<details class="full-qc-details"><summary>查看全流程质控（6 个环节 · ${run.summary.total} 个独立问题）</summary><div><div class="full-qc-domains">${run.domains.map((domain)=>domainCard(domain,true)).join('')}</div>${otherIssues.length?otherIssues.map((issue)=>issueRow(issue,true)).join(''):'<p class="full-qc-more">其余环节当前没有额外问题。</p>'}</div></details></section>`;
  }
...
function apply(){if(applying)return;const page=activePage();if(!page)return;const content=document.querySelector('.page-content');if(!content||content.querySelector('[data-full-chain-qc]'))return;const ctx=window.medicalSystemMvp?.getEpisodeContext?.();const episode=ctx?.episode;if(!episode)return;applying=true;try{const docs=readPatientDocumentSnapshots(episode);const facts=buildClinicalFactContext({episode,documentSnapshots:docs});const workspace=workspaceFor(episode.episodeId);const groupingRun=workspace?.formalGroupingRun||workspace?.preGroupingRun||null;const settlement=buildSettlementList(episode,{sourceDocuments:docs});const run=runFullChainQualityControl({episode,documentSnapshots:docs,factContext:facts,settlement,groupingRun,auditRun:workspace?.auditRun||null});const factCard=content.querySelector('[data-fact-platform-card]');const anchor=factCard||content.querySelector('.op-source')||content.firstElementChild;if(anchor)anchor.insertAdjacentHTML('afterend',render(run,page==='GROUPING'));else content.insertAdjacentHTML('afterbegin',render(run,page==='GROUPING'));}finally{applying=false;}}
