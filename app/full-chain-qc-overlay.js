...
function render(run, groupingOnly=false){
  if(groupingOnly){
    const relevant=run.issues.filter((issue)=>issue.qcDomain==='GROUPING'||issue.impactScope?.includes('GROUPING'));
    ...
    return `<section class="base-card full-qc-card" data-full-chain-qc><div class="base-card__header"><div><h3>分组前质控</h3><p>${esc(summary)}仅展示影响当前 DRG/DIP 的事项。</p></div><div class="full-qc-head-status"><strong>${relevant.length}</strong><span>影响分组</span><em class="status-badge status-badge--${tone(status)} status-badge--compact">${esc(status)}</em></div></div>${relevant.length?`<div class="full-qc-issues">${relevant.map(issueRow).join('')}</div>`:'<div class="fact-ok">关键输入检查无已知阻断项；请执行院内预分组查看规则结果。</div>'}<details class="full-qc-details"><summary>查看全流程质控（6 个环节 · ${run.summary.total} 个独立问题）</summary><div><div class="full-qc-domains">${run.domains.map(domainCard).join('')}</div>${otherIssues.length?otherIssues.map(issueRow).join(''):'<p class="full-qc-more">其余环节当前没有额外问题。</p>'}</div></details></section>`;
  }
